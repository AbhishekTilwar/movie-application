...
async function searchMovies() {
    ...
    movies.forEach(async (movie) => {
        ...
        const summaryPlot = `${moviesListData.Plot.substring(0, 110)}<span id=${hideReadMore}>...<button_p class="black read-more" onclick="showCompletePlot(${readMoreMovieID}, ${hideReadMore})">Read more</button_p></span>`
                ...
                <div class="cards">
                    <div class="card" id=${movieID}>
                        ...
                        <div class="card-header">
                            <a1 class="card-title">${moviesListData.Title}</a1>
                            <img src="images/star-icon.svg" class="star-icon" />
                            <span class="card-rating">${moviesListData.imdbRating}</span>
                        </div>
                        
                        <div class="card-meta">
                            <a class="card-runtime">${moviesListData.Runtime}</a>
                            <a>${moviesListData.Genre}</a>

                            <button_p class="card-btn card-watchlist watchlist-btn" id="${watchlistBtnKey}" onclick="addToWatchlist(${movieIDkey}, ${movieID}, ${watchlistBtnKey}, ${removeBtnKey})"><img src="images/watchlist-icon.svg" alt="Add film to Playlist" class="card-watchlist-plus-icon" />&nbsp;Playlist</button_p>

                            <button_p class="card-btn card-watchlist remove-watchlist-btn" id="${removeBtnKey}" onclick="removeFromWatchlist(${movieIDkey}, ${removeBtnKey}, ${watchlistBtnKey}, ${removeBtnKey})"><img src="images/remove-icon.svg" alt="Remove film to Playlist" class="card-watchlist-plus-icon" />&nbsp;Remove</button_p>
                        </div>
